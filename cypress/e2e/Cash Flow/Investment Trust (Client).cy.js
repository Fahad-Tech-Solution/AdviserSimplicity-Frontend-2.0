...
class ClientInvestmentTrust {
  section() {
    ...
    //   cy.get("#openingCashAtBank").should("have.value", "$4,536");
    ...
    // //Partner SectionTerm Deposit Inner Footer

    // cy.contains("Close").should("be.visible");
    // cy.contains("Submit").should("be.visible").click();

    // cy.get("#clientcf_WestFamilyTrustTermDeposits").should(
    //   "have.value",
    //   "$4,536"
    // );
...
//   cy.get('#clientcf_FamilyTrustPlatformInvestment').should("have.value", "$24");
...
//   cy.get('#clientcf_FamilyTrust').should("have.value", "$24");
 ...
 cy.get(":nth-child(8) > .py-4").within(() => {
   cy.contains("Family Trust Investment Properties");
   cy.get("img");
   cy.contains("Market Value");
   cy.contains("Loan Balance");
 });
 cy.get(
   ":nth-child(8) > .py-4 > .flex-column > .row > :nth-child(1) > .d-flex > .mb-0"
 ).click();
 cy.get(".modal-content").within(() => {
   cy.contains("Family Trust Investment Properties");
   cy.get(".btn-close");
 ...
 //Family Trust Investment Properties Footer
 cy.contains("Close").should("be.visible");
 cy.contains("Submit").should("be.visible").click();
...
export default ClientInvestmentTrust;
