class EstatePlanningAndProfessionalAdviser {
  section() {
    ...
    cy.get('[statusstep="40"] > .ant-steps-item-container > .ant-steps-item-icon > .ant-steps-icon > .rounded-circle').click();
...
//     cy.get('.css-1lx7dxn').clear().type("Aiden Smith{enter}");
//     //Will Card Section
...
//     cy.get(':nth-child(6) > .css-b62m3t-container > .css-14gfs0a-control > .css-d07bj1 > :nth-child(1)').click()
//     cy.get('.css-1f8fajx > .css-1lx7dxn').type("Child{enter}");
...
//     cy.get("#clientPOA").should("not.have.value", "");
 ...
 cy.contains("Emma Taylor");
    ...
    cy.get(".modal-body").within(() => {
      cy.contains("How many Professional Advisers does Aiden Smith have :").should(
        "be.visible"
      );
    });
    ...
    // cy.contains("Close").should("be.visible");
    cy.get('.modal-footer > .bgColor').contains("Submit").should("be.visible").click();
