...
const CFQModal = (props) => {
  ...
  let onSubmit = async (values) => {
    ...
    try {
      ...
      if (!bankAccountArray) {
        res = await PostAxios(
          `${DefaultUrl}/api/CF/cf_basicQuestions/Add`,
          obj
        );
      } else {
        obj._id = CFObject._id;
        res = await PatchAxios(
          `${DefaultUrl}/api/CF/cf_basicQuestions/Update`,
          obj
        );
      }
  ...
  const QuestionClick = (index, elem, values, setFieldValue) => {
    ...
    // Exclusive logic for income fields
    if (elem.key === "cf_incomeFromPartnership") {
      setFieldValue("cf_incomeFromSoleTrade", "No");
    } else if (elem.key === "cf_incomeFromSoleTrade") {
      setFieldValue("cf_incomeFromPartnership", "No");
    }
  };
