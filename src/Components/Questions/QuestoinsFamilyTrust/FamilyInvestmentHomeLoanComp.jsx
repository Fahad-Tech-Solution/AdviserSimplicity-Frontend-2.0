...
const FamilyInvestmentHomeLoanComp = (props) => {
  ...
  useEffect(() => {
    ...
    if (familyInvestmentPropertiesLoan[props.modalObject.Input] && familyInvestmentPropertiesLoan[props.modalObject.Input].length) {

      let arr = []

      for (let i = 0; i < familyInvestmentPropertiesLoan[props.modalObject.Input].length; i++) {
        arr.push("");
      }
  ...
  const fillInitialValues = (setFieldValue) => {
    ...
    if (familyInvestmentPropertiesLoan[props.modalObject.Input] && familyInvestmentPropertiesLoan[props.modalObject.Input].length) {
      ...
      familyInvestmentPropertiesLoan[props.modalObject.Input].forEach((data, i) => {
        if (data) {
  ...
  let onSubmit = async (values) => {
    ...
    // Check if familyInvestmentPropertiesLoan and the array at props.modalObject.Input exist
    // const bankAccountArray = familyInvestmentPropertiesLoan[props.modalObject.Input] || [];
    const bankAccountArray = familyInvestmentPropertiesLoan.clientFK || "";

    try {
      let res;
      if (!bankAccountArray) {
        res = await PostAxios(`${DefaultUrl}/api/familyInvestmentPropertiesLoan/Add`, obj);
      } else {
        obj.collection = props.modalObject.Input
        obj._id = familyInvestmentPropertiesLoan._id;
        console.log(obj)
        res = await PatchAxios(`${DefaultUrl}/api/familyInvestmentPropertiesLoan/Update`, obj);
      }

      if (res) {
        console.log(res);
        const updatedData = { ...questionDetail, familyInvestmentPropertiesLoan: res };
...
export default FamilyInvestmentHomeLoanComp;
