...
const PersonalInsuranceTPD = (props) => {
  ...
  let TPD = Object.keys(questionDetail.TPD).length > 0 ? questionDetail.TPD : {
    client: [],
    partner: [],
    joint: [],

  }; // Use an empty object as default if TPD is undefined

  const fillInitialValues = (setFieldValue) => {
    console.log("hello");
    if (
      TPD[props.modalObject.Input] &&
      TPD[props.modalObject.Input].owner
    ) {
      let data = TPD[props.modalObject.Input];
  ...
  let onSubmit = async (values) => {
    ...
    try {
      ...
      if (
        !questionDetail.TPD ||
        !questionDetail.TPD.clientFK
      ) {
        res = await PostAxios(`${DefaultUrl}/api/TPD/Add`, Obj);
      } else {
        Obj.collection = props.modalObject.Input;
        res = await PatchAxios(
          `${DefaultUrl}/api/TPD/Update`,
          Obj
        );
      }
      ...
      if (res) {
        console.log(res);
        const updatedData = { ...questionDetail, TPD: res };
    ...
    <div>
      <Modal size="lg" show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>TPD (Super Linked Policy)</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Formik
...
export default PersonalInsuranceTPD;
