...
const PersonalInsuranceTrauma = (props) => {
  ...
  const fillInitialValues = (setFieldValue) => {
    console.log("hello");
    if (
      trauma[props.modalObject.Input] &&
      trauma[props.modalObject.Input].owner
    ) {
      let data = trauma[props.modalObject.Input];
  ...
  useEffect(() => {
    // if (trauma[props.modalObject.Input] && trauma[props.modalObject.Input].length) {
    //     let arr = []
    //     for (let i = 0; i < trauma[props.modalObject.Input].length; i++) {
    //         arr.push("");
    //     }
  ...
  //   if (
  //     trauma[props.modalObject.Input] &&
  //     trauma[props.modalObject.Input].length
  //   ) {
  //     trauma[props.modalObject.Input].forEach((data, i) => {
  //       if (data) {
  ...
  let onSubmit = async (values) => {
    ...
    try {
      let res;
      if (
        !questionDetail.trauma ||
        !questionDetail.trauma.clientFK
      ) {
        res = await PostAxios(`${DefaultUrl}/api/trauma/Add`, Obj);
      } else {
        Obj.collection = props.modalObject.Input;
        res = await PatchAxios(
          `${DefaultUrl}/api/trauma/Update`,
          Obj
        );
      }
      ...
      if (res) {
        console.log(res);
        const updatedData = { ...questionDetail, trauma: res };
    ...
    <div>
      <Modal size="lg" show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Trauma (Super Linked Policy)</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Formik
...
export default PersonalInsuranceTrauma;
