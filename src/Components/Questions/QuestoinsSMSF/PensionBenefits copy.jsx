...
const PensionBenefits = (props) => {
    ...
    for (let i = 0; i < numberOfMaps; i++) {
      const newEntry = {
        ...
        originalPurchaseDate:
          values.pensionBenefits?.[i]?.originalPurchaseDate || "",
        eligibleServiceDate:
          values.pensionBenefits?.[i]?.eligibleServiceDate || "",
        ...
        LumpsumWithdrawalTaken:
          values.pensionBenefits?.[i]?.LumpsumWithdrawalTaken || "",
      };
  ...
  const FormulaSetting = (values, setFieldValue, currentInput, stakeHolder) => {
    try {
      ...
      let taxFree =
        parseFloat(
          values.pensionBenefits?.[index]?.taxFree?.replace(/[^0-9.-]+/g, "") ||
            0
        ) || 0;
      let currentBalance =
        parseFloat(
          values.pensionBenefits?.[index]?.currentBalance?.replace(
            /[^0-9.-]+/g,
            ""
          ) || 0
        ) || 0;
      ...
      setFieldValue(
        stakeHolder + `taxFreeComponent`,
        toCommaAndDollar(taxFreeComponent)
      );
      setFieldValue(
        stakeHolder + `taxableComponent`,
