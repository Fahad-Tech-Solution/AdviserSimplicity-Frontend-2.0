...
const SMSFQCards = (props) => {
    ...
    const SMSFDetailsSubmitted = (questionDetail.SMSFDetails && Object.keys(questionDetail.SMSFDetails).length > 0) ? true : false;
    ...
    let GetValue = () => {
        try {
            ...
            return toCommaAndDollar(SMSFAccumulationDetailsClientTotal + SMSFAccumulationDetailsPartnerTotal + SMSFAccumulationDetailsJointTotal + SMSFPensionPhaseClientTotal + SMSFPensionPhasePartnerTotal + SMSFPensionPhaseJointTotal);
    ...
    const reuseModal = ["SMSFBank", "SMSFTermDeposits", "SMSFAustralianShares", "SMSFManagedFunds"]; // add "Key" of Question on which you want to add Form in Cards only no pop ups

    return (
        <React.Fragment>
            {SmsFCards.map((SmsFElem, Pindex) => {
                const SMSFDetailsCard = SmsFElem.key === "SMSFDetails" ? true : false;
                const SMSFIP = SmsFElem.key === "SMSFInvestmentProperties" ? true : false;
                const SMSFReuse = reuseModal.includes(SmsFElem.key) ? true : false;
                const simpleWithOutJoin = SmsFElem.key === "SMSFInvestmentLoan" ? true : false;
                ...
                if (SMSFDetailsCard) {
                    ...
                    return (<SampleOne PartnerClass={PartnerClass} index={Pindex} jointClass={jointClass} elem={SmsFElem} OpenModal={OpenModal} arrayCount={arrayCount} />)
                }
                    ...
                    if (CRObject[SmsFElem.key] === "Yes") {
                        if (SMSFReuse) {
                            return (<div className={`col-md-3 mb-4`} key={index}>
                                <Card className="py-4 shadow borderOverAll GoalsobjectiveCard d-flex" style={{ borderRadius: "20px", height: "100%" }}>
                                    <h5 className='text-center' onClick={() => { console.log(questionDetail[SmsFElem.key]) }}>{SmsFElem.title}</h5>
                                    ...
                                    >
                                        <div className='col-12 p-0 '>
                                            <div className='d-flex flex-column-reverse justify-content-center align-items-center gap-2'>
                                                ...
                                                <label
                                                    className="mb-0 bg-secondary rounded-circle text-light py-1 px-2 curser-pointer"
                                                    onClick={() => { OpenReuseModal(SmsFElem.title, "client", SmsFElem.key) }}
                                                >
                                                    <div>
                                                        <FontAwesomeIcon icon={faArrowUpRightFromSquare} />
                                                    </div>
                            ...
                            return (
                                <React.Fragment key={Pindex}>
                                    <div className={`col-md-3 mb-4`}>
                                        ...
                                        <Card className="py-4 shadow borderOverAll GoalsobjectiveCard d-flex" style={{ borderRadius: "20px", height: "100%" }}>
                                            <h5 className='text-center' onClick={() => { console.log(questionDetail[SmsFElem.key]) }}>{SmsFElem.title}
                                            </h5>
                                            <div className='d-flex justify-content-center flex-column' style={{ marginTop: "auto" }}>
                                                ...
                                                <input type="text"
                                                    ...
                                                    value={questionDetail && questionDetail[SmsFElem.key]?.jointTotal ? questionDetail[SmsFElem.key].jointTotal : ""}
                                                />
                                            </div>
                                        </Card>
                                    </div>
