...
const SmsfPensionAccountMiddleWare = (props) => {
  ...
  const initialValues = {
    ...
    pensionData: existingMembers.length
  ...
  const CheckInputValue = (
    ...
    const ExpectedSum = parseFloat(
      pensionBenefitsTotalArray.reduce(
        (total, entry) =>
          total +
          parseFloat((entry.pensionBenefits || "0").replace(/[^0-9.-]+/g, "")),
        0
      )
    );
  ...
  let onSubmit = async (values) => {
    ...
    pensionData.forEach((item, index) => {
      ...
      if (item.member === "client") {
        ...
        obj.joint = [newEntry];
        obj.jointTotal = toCommaAndDollar(
          obj.joint.reduce(
            (total, entry) =>
              total +
              parseFloat(
          ...
          if (values.selectedMembers && values.selectedMembers.length > 0) {
            return values.selectedMembers.map((item, index) => ({
              ...
              pensionBenefitsTotal:
                values.pensionData[index]?.pensionBenefitsTotal || "",
              pensionBenefitsTotalArray:
                values.pensionData[index]?.pensionBenefitsTotalArray || [],
          ...
          <Form>
            ...
            <div className="d-flex justify-content-start align-items-center gap-4">
              <p
                ...
                onClick={() => {
                  console.log("Values :", values, "dataRows :", dataRows);
                }}
              >
                Members of SMSF
              </p>
