...
const SMSFInvestmentHomeLoanComp = (props) => {
  ...
  let SMSFInvestmentPropertiesLoan = (questionDetail && questionDetail.SMSFInvestmentPropertiesLoan && Array.isArray(questionDetail.SMSFInvestmentPropertiesLoan) && props && props.modalObject && typeof props.modalObject.index === 'number')
    ? questionDetail.SMSFInvestmentPropertiesLoan[props.modalObject.index] || { client: [], partner: [], joint: [] }
    : { client: [], partner: [], joint: [] };// Use an empty object as default if SMSFInvestmentPropertiesLoan is undefined


  let initialValues = SMSFInvestmentPropertiesLoan[props.modalObject.Input].length ? { NumberOfMap: SMSFInvestmentPropertiesLoan[props.modalObject.Input].length } : { NumberOfMap: "" };
  ...
  useEffect(() => {
    ...
    if (SMSFInvestmentPropertiesLoan[props.modalObject.Input] && SMSFInvestmentPropertiesLoan[props.modalObject.Input].length) {

      let arr = []

      for (let i = 0; i < SMSFInvestmentPropertiesLoan[props.modalObject.Input].length; i++) {
        arr.push("");
      }
  ...
  const fillInitialValues = (setFieldValue) => {
    ...
    if (SMSFInvestmentPropertiesLoan[props.modalObject.Input] && SMSFInvestmentPropertiesLoan[props.modalObject.Input].length) {
      ...
      SMSFInvestmentPropertiesLoan[props.modalObject.Input].forEach((data, i) => {
        if (data) {
  ...
  let onSubmit = async (values) => {
    ...
    // Check if SMSFInvestmentPropertiesLoan and the array at props.modalObject.Input exist
    // const bankAccountArray = SMSFInvestmentPropertiesLoan[props.modalObject.Input] || [];
    const bankAccountArray = SMSFInvestmentPropertiesLoan.clientFK || "";

    try {
      let res;
      if (!bankAccountArray) {
        res = await PostAxios(`${DefaultUrl}/api/SMSFInvestmentPropertiesLoan/Add`, obj);
      } else {
        obj.collection = props.modalObject.Input
        obj._id = SMSFInvestmentPropertiesLoan._id;
        console.log(obj)
        res = await PatchAxios(`${DefaultUrl}/api/SMSFInvestmentPropertiesLoan/Update`, obj);
      }

      if (res) {
        console.log(res);
        const updatedData = { ...questionDetail, SMSFInvestmentPropertiesLoan: res };
...
export default SMSFInvestmentHomeLoanComp;
