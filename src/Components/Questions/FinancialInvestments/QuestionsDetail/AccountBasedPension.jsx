...
const AccountBasedPension = (props) => {
  ...
  try {
    const targetArray = accountBasedPensionIssues?.[props.modalObject?.Input];
    if (Array.isArray(targetArray)) {
      numberOfMap = targetArray.length;
    }
  } catch (error) {
    console.error("Error accessing accountBasedPensionIssues:", error);
  }
  ...
  let onSubmit = async (values) => {
    ...
    let total = newEntries.reduce(
      (total, entry) =>
        total + parseFloat(entry.pensionPayment.replace(/[^0-9.-]+/g, "")),
      0
    );
            ...
            <Row>
              ...
              >
                {modalObject.key === "portfolioArray" ? (
                  ...
                  <AccountBasedBalance /> // is ko change karna hai
                ) : modalObject.key === "beneficiariesArray" ? (
                  <Beneficiaries /> // is ko change karna hai
                                ...
                                <td>
                                  ...
                                  >
                                    ...
                                    bankDetailObj.AccountBasedPensions.length >
                                      ...
                                      <option disabled>
                                        No Platforms Added in Account Based
                                        Pensions
                                      </option>
                                    )}
                                  </Field>
