...
const HomeLoan = (props) => {
    ...
    let familyHomeLoan = Object.keys(questionDetail.familyHomeLoan).length > 0 ? questionDetail.familyHomeLoan : {
        client: [],
        partner: [],
        joint: [],

    }; // Use an empty object as default if familyHomeLoan is undefined


    let initialValues = familyHomeLoan[props.modalObject.Input].length ? { NumberOfMap: familyHomeLoan[props.modalObject.Input].length } : { NumberOfMap: "" };
    ...
    useEffect(() => {
        ...
        if (familyHomeLoan[props.modalObject.Input] && familyHomeLoan[props.modalObject.Input].length) {

            let arr = []

            for (let i = 0; i < familyHomeLoan[props.modalObject.Input].length; i++) {
                arr.push("");
            }
    ...
    const fillInitialValues = (setFieldValue) => {
        ...
        if (familyHomeLoan[props.modalObject.Input] && familyHomeLoan[props.modalObject.Input].length) {
            ...
            familyHomeLoan[props.modalObject.Input].forEach((data, i) => {
                if (data) {
    ...
    let onSubmit = async (values) => {
        ...
        // Check if familyHomeLoan and the array at props.modalObject.Input exist
        // const bankAccountArray = familyHomeLoan[props.modalObject.Input] || [];
        const bankAccountArray = familyHomeLoan.clientFK || "";

        try {
            let res;
            if (!bankAccountArray) {
                res = await PostAxios(`${DefaultUrl}/api/familyHomeLoan/Add`, obj);
            } else {
                obj.collection = props.modalObject.Input
                res = await PatchAxios(`${DefaultUrl}/api/familyHomeLoan/Update`, obj);
            }

            if (res) {
                console.log(res);
                const updatedData = { ...questionDetail, familyHomeLoan: res };
