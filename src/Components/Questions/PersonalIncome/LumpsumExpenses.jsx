...
const LumpsumExpenses = (props) => {
  ...
  let initialValues = incomeFromLumpsumExpense[props.modalObject.Input].length
    ? { NumberOfMap: incomeFromLumpsumExpense[props.modalObject.Input].length }
    : { NumberOfMap: "" };
  ...
  useEffect(() => {
    if (
      incomeFromLumpsumExpense[props.modalObject.Input] &&
      incomeFromLumpsumExpense[props.modalObject.Input].length
    ) {
      let arr = [];

      for (
        let i = 0;
        i < incomeFromLumpsumExpense[props.modalObject.Input].length;
        i++
      ) {
        arr.push("");
      }
  ...
  let onSubmit = async (values) => {
    ...
    // Check if incomeFromLumpsumExpense and the array at props.modalObject.Input exist
    // const bankAccountArray = incomeFromLumpsumExpense[props.modalObject.Input] || [];
    const bankAccountArray = incomeFromLumpsumExpense.clientFK || "";

    try {
      let res;
      if (!bankAccountArray) {
        res = await PostAxios(
          `${DefaultUrl}/api/incomeFromLumpsumExpense/Add`,
          obj
        );
      } else {
        obj.collection = props.modalObject.Input;
        res = await PatchAxios(
          `${DefaultUrl}/api/incomeFromLumpsumExpense/Update`,
          obj
        );
      }
      ...
      if (res) {
        console.log(res);
        const updatedData = { ...questionDetail, incomeFromLumpsumExpense: res };
...
export default LumpsumExpenses;
