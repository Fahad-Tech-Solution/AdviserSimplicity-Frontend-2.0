...
import { GoalsDetail } from '../../Store/Store';
...
const GoalsObjectiveNew = () => {
    ...
    let allGoals = [
        ...
        },
        {
            ...
            whenScopeIs: "Debt Management",
            descriptionArray: [
                { text: "I/We would like Reduce our/my current home loan as soon as possible /ahead of retirement." },
                { text: "I/We want to know how much I/We need  to pay off our/my home loan  each week/fortnight/month so I/we can be debt free in retirement/pay it off in the next XX Years" },
            ]
        },
        {
    ...
    let getInitialValues = (setFieldValue, values) => {
        if (goalsDetail?.GoalsKey && goalsDetail.GoalsKey?.scopeOfAdvice) {
            setFieldValue("scopeOfAdvice", goalsDetail.GoalsKey.scopeOfAdvice || "")
            setFieldValue("when", goalsDetail.GoalsKey.when || "")
            setFieldValue("estimatedValue", goalsDetail.GoalsKey.estimatedValue || "")
            setFieldValue("description", goalsDetail.GoalsKey.description || "")
        }
        else {
            setFieldValue("scopeOfAdvice", "")
            setFieldValue("when", "")
            setFieldValue("estimatedValue", "")
                        ...
                        <div className={`col-md-4 mb-4`} key={index}>
                            <Card className="py-4 shadow px-4" style={{ borderRadius: "20px", height: "100%" }}>
                                ...
                                <div className='d-flex justify-content-center align-items-center gap-3 '>
                                    <div className=''>
                                        <h5 className='text-center mt-2' onClick={() => { console.log(goalsDetail[elem.key]) }}>
                                            {elem.title}
                                        </h5>
                                    </div>
                                    {elem.modalBtn === "head" &&
                                ...
                                >
                                    {({ values, setFieldValue }) => {
                                        ...
                                        // useEffect(() => {
                                        //     getInitialValues(setFieldValue, goalsDetail[elem.key])
                                        // }, [goalsDetail[elem.key]])
                                        ...
                                        return (<Form>
                                            ...
                                            <Field
                                                ...
                                                name={"estimatedValue"}
                                                readOnly
                                                value={goalsDetail[elem.key]?.estimatedValue || ""}
                                            />
